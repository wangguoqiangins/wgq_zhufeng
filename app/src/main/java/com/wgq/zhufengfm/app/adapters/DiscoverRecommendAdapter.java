...
import com.wgq.zhufengfm.app.model.discover.*;
import com.wgq.zhufengfm.app.tasks.ImageLoadTask;
...
public class DiscoverRecommendAdapter extends BaseAdapter {
    ...
    public Object getItem(int position) {
        ...
        switch (position) {
            case 0:
                ret = recommend.getEditorRecommendAlbums();//小编推荐
                break;
            case 1:
                ret = recommend.getSpecialColumn();//精品听单
                break;
            case 2:
                ret = recommend.getDiscoverColumns();//发现新奇
                break;
            default:
                HotRecommends hotRecommends = recommend.getHotRecommends();
                if (hotRecommends != null) {
    ...
    private View bindHotColumnView(Object item, View convertView, ViewGroup parent) {
        ...
        //对于热门推荐，更多点击的时候，对象中包含categoryid，通过这个
        //作为点击事件的入口
        holder.txtMore.setTag(Constants.TAG_DISCOVER_HOT_RECOMMEND +":"+hot.getCategoryId());
